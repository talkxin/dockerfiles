...
QT_CONFIGURE_OPTS="-prefix $QT_OUTPUT -release -opensource -xplatform linux-arm-gnueabi-g++ -no-opengl -no-icu -no-eglfs -no-iconv -nomake tests -nomake examples -linuxfb -I$TSLIB_OUTPUT/include -L$TSLIB_OUTPUT/lib"
...
#下载并解压tslib库
wget -O tslib $TSLIB_PATH && tar xvf tslib
rm -rf tslib
TSLIB_VERSION=`ls | grep tslib | awk 'NR==1{print $1}'`

#下载并解压QT
wget -O qt-everywhere $QT_EVERYWHERE && tar xvf qt-everywhere
rm -rf qt-everywhere
